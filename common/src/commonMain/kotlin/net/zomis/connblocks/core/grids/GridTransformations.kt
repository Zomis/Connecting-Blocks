package net.zomis.connblocks.core.grids

import net.zomis.connblocks.core.GreedyIterator
...
enum class Transformation(private val transformations: List<TransformationType>) {
    ...
    fun apply(transformation: Transformation): Transformation {
        ...
        val sum = this.transformations + transformation.transformations
        val transformationsByType = sum.groupingBy { it }.eachCount().withDefault { 0 }
        val flipX = transformationsByType.getValue(TransformationType.FLIP_X) % 2
        val flipY = transformationsByType.getValue(TransformationType.FLIP_Y) % 2
        val rotations = transformationsByType.getValue(TransformationType.ROTATE) % 4
...
fun <T> Grid<T>.standardizedTransformation(valueFunction: (T) -> Int): Transformation {
    ...
    while (possibleTransformations.size > 1 && position != null) {
        val best = GreedyIterator<Transformation>()
        val vf: (Transformation) -> Double = { transformation ->
        ...
        possibleTransformations.forEach {
            best.next(vf.invoke(it)) { it }
        }
        possibleTransformations.retainAll(best.getBest().toSet())
